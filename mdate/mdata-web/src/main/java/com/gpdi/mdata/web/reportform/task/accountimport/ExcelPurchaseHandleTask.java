 ...
 * @description:采购合同导入excel后处理数据任务
 */
public class ExcelPurchaseHandleTask implements Callable<Boolean>,Cloneable {

    private static final Logger logger = LoggerFactory.getLogger(ExcelPurchaseHandleTask.class);

    private List<PurchaseContract> list = new LinkedList<>();
    private Integer parentId;

    public ExcelPurchaseHandleTask(List<PurchaseContract> list, Integer parentId) {
        this.list = list;
        this.parentId = parentId;
    }

    @Override
    public ExcelPurchaseHandleTask clone() throws CloneNotSupportedException {
        return (ExcelPurchaseHandleTask)super.clone();
    }
    ...
    * */
    ...
    public Boolean call() {
        try{
            PurchaseParentService parentService = BeanUtils.getBean(PurchaseParentService.class);
            if(parentService != null){
                boolean result = parentService.saveOrderHandle(list,parentId);
                if(result){
