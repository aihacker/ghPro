...
<div class="main">
    ...
    <label>经办部门名称:</label><input type="text" value="${queryResult.rows[0].undertake_dept}">
    <label>签订合同数量:</label><input type="text" value="">
    <label>采购方式:</label><input type="text" value="${queryResult.rows[0].purchase_way}">
    <div class="add-table col-xs-12">
        ...
        <table id="myTable" class="table table-bordered tablesorter" style="width: 50%;float: right">
            <thead align="center">
            <tr align="center">
                <th>合同类型</th>
                <th>合同数量</th>
            ...
            <c:forEach items="${temp}" var="tp" varStatus="status">
                <tr>

                    <td>${tp.getContractTypeName()}</td>
                    <td><a href="${home}/reportform/form/partcontractnumber/query.html?type=${tp.getContractTypeName()}&&dept=${tp.getUndertakeDept()}&&purchaseWay=${tp.getPurchaseWay()}">${tp.getContractCode()}</a></td>
