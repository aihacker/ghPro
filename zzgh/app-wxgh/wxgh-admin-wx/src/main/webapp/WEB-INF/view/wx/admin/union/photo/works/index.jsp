                                    ...
                                    <c:choose>
                                        <c:when test="${item.status == 1}">
                                            <button class="mui-btn mui-btn-blue mui-pull-right"
                                                    style="margin: 10px; background: red; border:red;"
                                                    data-id="${item.id}" onclick="del(this);">删除
                                            </button>
                                        </c:when>
                                        <c:when test="${item.status == 2}">
                                            <button class="mui-btn mui-btn-blue mui-pull-right"
                                                    style="margin: 10px"
                                                    data-id="${item.id}" onclick="reback(this);">恢复
                                            </button>
                                        </c:when>
                                    </c:choose>
...
<script type="text/javascript">
    ...
    function del(obj) {
        ...
        ui.confirm('是否删除该作品?', function () {
            wxgh.request.post("update.json", {id:id, status:2}, function () {
                ui.alert('删除成功')
    ...
    function reback(obj) {
        var id = $(obj).attr('data-id')
        ui.confirm('是否恢复该作品?', function () {
            wxgh.request.post("update.json", {id:id, status:1},function () {
                ui.alert('恢复成功')
                window.location.reload()
            })
        })
