...
public class VerifyUtils {
    ...
    @Autowired
    private RuleExpectionService ruleExpectionService;
   ...
   /* @Autowired
     ...
     * @return
     */
    public String verify(PlaceYuyue yuyue) {
        String msg = null;
...
*/
        //验证规则
        List<PlaceRule> rules = placeRuleService.getRules(1);
        if (!TypeUtils.empty(rules)) {
            for (PlaceRule r : rules) {
                if (r.getType() == PlaceRule.TYPE_NUMB) { //数量限制
                    String yan = verifyNumb(r, yuyue.getUserid());
                    if (null != yan) {
                        msg = yan;
                        break;
                    }
                } else if (r.getType() == PlaceRule.TYPE_SITE) { //场地限制
     ...
     * @return
     */
    private String verifyNumb(PlaceRule rule, String userid) {
        String msg = null;
        ...
        //再进行例外验证
        List<RuleExpection> expections = ruleExpectionService.getExpections(rule.getId());
        if (!TypeUtils.empty(expections)) {
            Map<String, RuleExpection> map = toMap(expections);
            RuleExpection userExpection = map.getOrDefault(userid, null);
    ...
    private String verifyTime(PlaceRule rule, String userid, Integer timeId) {
        String msg = null;
        String ruleStr = rule.getRule();
        ...
        PlaceTime t = placeTimeService.getPlaceTime(r.getTimeId());
        if (t != null) {
            Date today = new Date();
    ...
    private Map<String, RuleExpection> toMap(List<RuleExpection> expections) {
        Map<String, RuleExpection> map = new HashMap<>();
        for (RuleExpection e : expections) {
