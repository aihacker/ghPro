...
import wxgh.sys.dao.canteen.act.CanteenActDao;
...
public class CanteenActService {
    ...
    public void cancelAct(Integer id) {
        //todo 更新活动状态
        String sql = "update t_canteen_act set status = ? where id = ?";
        pubDao.execute(sql, Status.FAILED.getStatus(), id);

        //todo 停止定时器
        CanteenAct act = pubDao.query(CanteenAct.class, "select act_id, regular from t_canteen_act where id = ?", id);
        if (act != null) {
    ...
    public String getDetails(Integer actId) {
        String sql = "select info from t_canteen_act where id = ?";
        return pubDao.query(String.class, sql, actId);
    }

    public String getName(String actId) {
        String sql = "select name from t_canteen_act where act_id = ?";
        return pubDao.query(String.class, sql, actId);
    }

    public String getGroupId(Integer actId) {
    ...
    public ScoreRule getScoreRule(Integer actId) {
        SQL sql = new SQL.SqlBuilder()
                .table("score_rule r")
                .field("r.*")
                .join("canteen_act a", "a.score_rule = r.rule_id")
    ...
    public CanteenAct getBaseActInfo(Integer actId) {
        SQL.SqlBuilder sqlBuilder = new SQL.SqlBuilder()
                .field("*")
                .table("canteen_act")
                .where("id = ?")
                .select();
        return pubDao.query(CanteenAct.class, sqlBuilder.build().sql(), actId);
    }

    public CanteenAct getBaseInfo(String actId) {
        SQL.SqlBuilder sqlBuilder = new SQL.SqlBuilder()
                .field("*")
                .table("canteen_act")
                .where("act_id = ?")
    ...
    public ActInfo getActInfo(Integer actId) {
        ...
        if (info != null) {
            ...
            if (info.getRegular() == 0) {
                sql.field("(select COUNT(*) FROM t_canteen_act_join where act_id = a.act_id and type = 1) as joinNumb")
                        .field("(select COUNT(*) FROM t_canteen_act_join where act_id = a.act_id and type = 2) as leaveNumb")
                        .field("(select type from t_canteen_act_join where act_id = a.act_id and userid = :userid limit 1) as joinType")
                ...
                sql.field("(select COUNT(*) FROM t_canteen_act_join where act_id = a.act_id and dateid=:dateid and type = 1) as joinNumb")
                        .field("(select COUNT(*) FROM t_canteen_act_join where act_id = a.act_id and dateid=:dateid and type = 2) as leaveNumb")
                        .field("(select type from t_canteen_act_join where act_id = a.act_id and userid = :userid and dateid=:dateid limit 1) as joinType");
            }
    ...
    @Transactional
    public void updateActTime(CanteenAct act, Integer push) {
        String sql = "update t_canteen_act a set a.start_time = :startTime,a.end_time = :endTime where a.act_id = :actId ";
    ...
    @Transactional
    public void addAct(CanteenAct act, boolean push) {
        String actId = StringUtils.uuid();
        act.setActId(actId);
        // 协会发布活动页面传递过来的是主键id, 团队活动发布传的是groupId
        if (act.getActType().equals(CanteenAct.ACT_TYPE_CANTEEN)) {
            String sql = "select group_id from t_canteen where id = ? limit 1 ";
    ...
    public List<ActList> listAct(ActParam param) {
        String sql1="select c.group_id from t_canteen c where c.id=?";
        String groupId=pubDao.query(String.class,sql1,Integer.parseInt(param.getGroupId()));
        ...
        if (param.getGroupId() != null) {
            sql.join("canteen g", "g.group_id = a.group_id")
    ...
    public List<ActList> userListJoinAct(ActParam param){
        SQL.SqlBuilder sql = new SQL.SqlBuilder()
                .table("canteen_act a")
                .field("a.id, a.name, a.address, a.phone, a.money,a.info,a.act_numb,a.act_id,a.link")
                .field("s.file_path as path, s.thumb_path as thumb")
                .join("sys_file s", "s.file_id = a.img_id", Join.Type.LEFT)
                .join("canteen_act_join j","j.act_id=a.act_id", Join.Type.INNER)
                .order("a.add_time", Order.Type.DESC);
        if (param.getStatus() != null) {
            sql.where("a.status = :status");
        }
        ...
        if (param.getRegular() != null) {
            if (param.getRegular() == 1) { //如果为定期活动
                sql.field("(select CONCAT('周', GROUP_CONCAT(week_name(`week`) SEPARATOR '、'), DATE_FORMAT(a.start_time,'%H:%i'),'-', DATE_FORMAT(a.end_time,'%H:%i')) from t_act_regular where act_id = a.act_id)as time")
                        .where("a.regular = :regular and j.dateid>=DATE_FORMAT(now(),'%Y%d')");
            } else if (param.getRegular() == 0) {
                sql.field("CONCAT(DATE_FORMAT(a.start_time,'%m-%d %H:%i'),'-', DATE_FORMAT(a.end_time,'%m-%d %H:%i')) as time")
                        .where("a.regular = :regular and a.end_time>=now()");
            } else {
                sql.field("if(a.regular = 1, (select CONCAT('周', GROUP_CONCAT(week_name(`week`) SEPARATOR '、'), DATE_FORMAT(a.start_time,'%H:%i'),'-', DATE_FORMAT(a.end_time,'%H:%i')) from t_act_regular where act_id = a.act_id), CONCAT(DATE_FORMAT(a.start_time,'%m-%d %H:%i'),'-', DATE_FORMAT(a.end_time,'%m-%d %H:%i'))) as time");
            }
        }
