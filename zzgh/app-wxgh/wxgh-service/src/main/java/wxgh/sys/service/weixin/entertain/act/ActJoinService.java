...
public class ActJoinService {
    ...
    public void actJoinBig(Integer actId, Integer type) {
        ...
        //获取活动类型
        Act act = pubDao.query(Act.class, "select * from t_act where id = ?", actId);
        ...
        ActJoin actJoin = pubDao.query(ActJoin.class, seSql.sql(), actId, userid);
        if (actJoin == null) { //保存
            if(type == 1){
                //判断报名人数
            ...
            String sql="";
            if(type == 3){
                sql ="delete from t_act_join where id=?";
                pubDao.execute(sql,actJoin.getId());
            }else{
                sql = "update t_act_join set type = ?, add_time = ? where id = ?";
    ...
    public void actJoin(Integer actId, Integer type) {
        String userid = UserSession.getUserid();
        //获取活动类型
        Act act = pubDao.query(Act.class, "select a.group_id,a.money,a.act_id,a.has_score,a.score_rule, a.act_numb, a.act_type, a.regular, a.start_time, a.end_time, if(a.regular=1, (SELECT GROUP_CONCAT(`week`) from t_act_regular where a.act_id = act_id order by `week`),NULL) as `week` from t_act a where a.id = ?", actId);
        if (Act.ACT_TYPE_GROUP.equals(act.getActType())) {
            // 判断用户是否为协会成员
            String groupSql = "select status from t_group_user where userid = ? and group_id = (select group_id from t_act where id = ?)";
        ...
        } else if (Act.ACT_TYPE_TEAM.equals(act.getActType())) {
            String teamSql = "select status from t_chat_user where userid = ? and group_id = (select group_id from t_act where id = ?)";
        ...
        if (act.getRegular() == 0) {
            ...
            actJoin = pubDao.query(ActJoin.class, seSql.build().sql(), actId, userid);
        } else {
            dateId = DateUtils.getNearDate(act.getStartTime(), act.getWeek());
            seSql.where("a.id = ? and j.userid = ? and j.dateid = ?");
            actJoin = pubDao.query(ActJoin.class, seSql.build().sql(), actId, userid, dateId);
        }
...
//            }
        ...
        if(actJoin != null){
            ...
            if(actJoin.getType() != null){
                ...
                String updateSql = "update t_act_join set type = ?, add_time = ? where id = ?";
            ...
            //  插入数据
            String sql = "INSERT INTO t_act_join (userid, act_id, type, add_time, dateid)\n" +
                    "SELECT :userid, act_id, :type, :addTime, :dateid FROM t_act where id = :id";
            actJoin = new ActJoin();
            actJoin.setId(actId);
            actJoin.setUserid(userid);
            actJoin.setType(type);
        ...
        if (actJoin == null) { // 保存参加
            ...
            String sql = "update t_act_join set type = ?, add_time = ? where id = ?";
            pubDao.execute(sql, type, new Date(), actJoin.getId());
            actScoreService.update_union_score(act,userid,type);
        }
    }
    ...
    public List<JoinList> listJoins(JoinParam param) {
        ...
        if (param.getActId() != null) {
            sql.join("act a", "a.act_id = j.act_id")
                    .where("a.id = :actId");
        }
