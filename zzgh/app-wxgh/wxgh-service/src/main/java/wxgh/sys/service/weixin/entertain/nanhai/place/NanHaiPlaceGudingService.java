...
import wxgh.sys.dao.entertain.nanhai.place.NanHaiPlaceGudingDao;
...
public class NanHaiPlaceGudingService {

    @Transactional
    public void save(NanHaiPlaceGuding guding) {
        guding.setAddTime(new Date());
        placeGudingDao.save(guding);

        String sql = "update t_nanhai_place_time set time_type=? where id=?";
        generalDao.execute(sql, 2, guding.getTimeId());
    }

    public Integer getStatus(Integer id) {
        String sql = "select status from t_nanhai_place_guding where id=?";
        return generalDao.query(Integer.class, sql, id);
    }

    public NanHaiPlaceGuding getOne(Integer timeId) {
        String sql = "select * from t_nanhai_place_guding where time_id=? limit 1";
        return generalDao.query(NanHaiPlaceGuding.class, sql, timeId);
    }
    ...
    public List<GudingList> getGudings(GudingParam query) {
        String sql = "select t.id as timeId, CONCAT(t.start_time,'-',t.end_time) as time, t.`week`,\n" +
                ...
                "from t_nanhai_place_guding g\n" +
                "join t_nanhai_place_time t on g.time_id = t.id\n" +
                "join t_nanhai_place_site s on t.site_id = s.id\n" +
                "join t_nanhai_place_cate c on t.cate_id = c.id where s.place_id=?";
    ...
    public void del(Integer timeId) {
        //删除固定场
        String sql = "delete from t_nanhai_place_guding where id = ?";
        generalDao.execute(sql, timeId);

        //判断时间是否过期
        sql = "select * from t_nanhai_place_time where id=?";
        NanHaiPlaceTime time = generalDao.query(NanHaiPlaceTime.class, sql, timeId);
        if (time != null) {
            Integer status;
...
//            sql = "update t_place_time set status=? where id=?";
            sql = "update t_nanhai_place_time set time_type=?,d_status=?,s_status=? where id=?";
            generalDao.execute(sql, 1, status, status, timeId);
        }
    }

    @Transactional
    public void edit(NanHaiPlaceGuding guding) {
        placeGudingDao.update(guding);
    }

    @Autowired
    private NanHaiPlaceGudingDao placeGudingDao;
