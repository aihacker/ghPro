...
import wxgh.sys.dao.entertain.nanhai.place.NanHaiPlaceYuyueDao;
...
public class NanHaiPlaceYuyueService {
   ...
   /* @Override
    ...
    public List<MeYuyue> getUserYuyue(Integer status, String userid) {
        String sql = "select y.id, t.start_time, t.end_time, t.`week`,\n" +
                ...
                "join t_nanhai_place_time t on y.time_id = t.id\n" +
                "join t_nanhai_place_site s on t.site_id = s.id\n" +
                "join t_nanhai_place_cate c on t.cate_id = c.id\n" +
                "join t_nanhai_place p on s.place_id = p.id\n" +
                "where y.`status` = ?\n" +
    ...
    public List<MeYuyue> getUserYuyue(YuYueRecordParam param) {
        SQL.SqlBuilder sql = new SQL.SqlBuilder()
                ...
                .join("t_nanhai_place_time t", "y.time_id = t.id")
                .join("t_nanhai_place_site s", "t.site_id = s.id")
                .join("t_nanhai_place_cate c", "t.cate_id = c.id")
                .join("t_nanhai_place p", "s.place_id = p.id")
...
//                Integer week=meYuyues.get(i).getWeek()+1;
    ...
    @Transactional
    public void saveYuyue(NanHaiPlaceYuyue placeYuyue) {
        placeYuyueDao.save(placeYuyue);
    }

    @Transactional
    public void saveList(List<NanHaiPlaceYuyue> placeYuyues) {
        for (NanHaiPlaceYuyue placeYuyue : placeYuyues) {
            placeYuyueDao.save(placeYuyue);
        }
    }
    ...
    public void delteYuyue(Integer id) {
        NanHaiPlaceYuyue yuyue = placeYuyueDao.get(id);
        if (yuyue != null) {

            //更新预约时间的状态
            String timeSql = "update t_nanhai_place_time set status=? where id = ?";
    ...
    public void delteYuyue(Integer week, Integer id) {
        ...
        if (yuyue != null) {
            ...
            //更新预约时间的状态
            String timeSql = isSingle ? "update t_nanhai_place_time set d_status=? where id = ?" : "update t_nanhai_place_time set s_status=? where id = ?";
            generalDao.execute(timeSql, 0, yuyue.getTimeId());
...
//            placeYuyueDao.delete(id);
            ...
            String sql = "delete from t_nanhai_place_yuyue where time_id = ? AND date_id >= ? AND date_id <= ?";
...
//            message.addUser("18402028708");
    ...
    public List<NanHaiPlaceYuyue> getList(PlaceParam placeQuery) {
    ...
    public int updateStatus(Integer yuyueId, Integer status) {
        String sql = "update t_nanhai_place_yuyue set status = ? where id = ?";
    ...
    public List<Integer> getTimeOutYuyue() {
        ...
        String sql = "select y.id from t_nanhai_place_yuyue y\n" +
                "join t_nanhai_place_time t on y.time_id = t.id\n" +
                "where y.date_id < ?\n" +
    ...
    public Integer countDay(String userid) {
        ...
        String sql = "select count(*) from t_nanhai_place_yuyue where userid = ?\n" +
                "and date_id = ?";
    ...
    public List<wxgh.data.entertain.place.YuyueList> getYuyueList(ApiParam apiParam) {
        String sql = "select t.`week`, CONCAT(t.start_time,'~', t.end_time) as time, t.id,\n" +
                ...
                "join t_nanhai_place_time t on y.time_id = t.id\n" +
                "join t_nanhai_place_site s on y.site_id = s.id\n" +
                "join t_nanhai_place_cate c on t.cate_id = c.id\n" +
                "join t_nanhai_place p on s.place_id = p.id ";
    ...
    public List<YuyueList> getYuyueList(Integer placeId, Integer cateId) {
        ...
        String sql = "SELECT y.pay_type AS payType, y.site_id, y.time_id, y.userid, y.`status`, s.`name`, t.start_time, t.end_time, t.`week`,u.name as username,u.mobile, d.name FROM t_nanhai_place_yuyue y\n" +
                "join t_nanhai_place_site s on y.site_id = s.id\n" +
                "join t_nanhai_place_time t on y.time_id = t.id\n" +
                "join t_user u on y.userid = u.userid\n" +
    ...
    public YuyueDetail getYuyueDetail(Integer id) {
        String sql = "select y.id, t.`week`, t.start_time, t.end_time, s.`name` as siteName, c.`name` as cateName, p.title as placeName, " +
                "y.userid, y.pay_type, y.pay_price, y.status, y.time_id from t_nanhai_place_yuyue y\n" +
                "join t_nanhai_place_time t on y.time_id = t.id\n" +
                "join t_nanhai_place_site s on y.site_id = s.id\n" +
                "join t_nanhai_place_cate c on s.cate_id = c.id\n" +
                "join t_nanhai_place p on s.place_id = p.id\n" +
                "where y.id = ?";
        return generalDao.query(YuyueDetail.class, sql, id);
    }

    public NanHaiPlaceYuyue getWeekYuyueByTime(Integer timeId, Integer status) {
        String sql = "select * from t_nanhai_place_yuyue\n" +
                "where time_id = ? and status = ?\n" +
                "AND date_id >= ? AND date_id <=?";
        Date today = new Date();
        int startInt = DateUtils.getFirstWeekDayInt(today);
        int endInt = DateUtils.getLastWeekDayInt(today);
        return generalDao.query(NanHaiPlaceYuyue.class, sql, timeId, status, startInt, endInt);
    }
    ...
    public YuyueInfo getYuyueInfoById(Integer timeId) {
        ...
        SQL sql = new SQL.SqlBuilder()
                .select("t_nanhai_place_yuyue as y")
    ...
    @Autowired
    private NanHaiPlaceYuyueDao placeYuyueDao;
