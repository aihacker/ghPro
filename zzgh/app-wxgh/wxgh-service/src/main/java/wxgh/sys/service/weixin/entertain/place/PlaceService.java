...
public class PlaceService {
    ...
    public List<YuyueList> getAllPlace(Integer placeId, Integer timeStatus) {
        String sql = "select p.title as placeName,\n" +
                ...
                "from t_place p\n" +
                "join t_place_site s on p.id = s.place_id\n" +
                "join t_place_cate c on c.id = s.cate_id\n" +
                "join t_place_time t on s.id = t.site_id\n" +
                "where p.id = ?";
        if (timeStatus != null) {
            if (timeStatus == -1) {
                sql += " and t.status != ?";
    ...
    public Integer countPlaces(PlaceParam query) {
        String sql = "select count(*) from t_place where find_in_set(?, type_int) and place_type=? and is_ad=? and status=?";
    ...
    public List<ListData> getIndexList(PlaceParam query) {
        String sql = "select id,title,type_int as cate,price,geohash,lat,lng from t_place where find_in_set(?, type_int)" +
        ...
        if (places != null && places.size() > 0) {
            sql = "select img_path from t_place_img where place_id=? and is_cover=? limit 1";
            String sql1 = "select name from t_place_cate where find_in_set(id, ?)>0 and status=1";
    ...
    public Place getAdPlace(Integer placeType, Integer cateId) {
        String sql = "select id,title,address,geohash,lat,lng,price,is_ad " +
                "from t_place where place_type=? and find_in_set(?, type_int) and status=1 and is_ad=? limit 1";
        Place place = generalDao.query(Place.class, sql, placeType, cateId, 1);
        if (place == null) {
            place = generalDao.query(Place.class, sql, placeType, cateId, 0);
        }
        if (place != null) {
            sql = "select img_path from t_place_img where place_id=? and is_cover=? limit 1";
    ...
    public List<NameValue> listCate(Integer placeId) {
        String sql = "select pc.id as value, pc.`name` from t_place_cate pc where FIND_IN_SET(id, (select type_int from t_place where id = ?))";
