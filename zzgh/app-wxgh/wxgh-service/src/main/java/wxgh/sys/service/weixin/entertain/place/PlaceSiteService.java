...
public class PlaceSiteService{

    public List<PlaceSite> getSites(Integer placeId, Integer cateId) {
        String sql = "select * from t_place_site where place_id=?";
        if (cateId != null) {
            sql += " and cate_id=? order by name";
            return generalDao.queryList(PlaceSite.class, sql, placeId, cateId);
        }
        return generalDao.queryList(PlaceSite.class, sql, placeId);
    }

    @Transactional
    public int updateSiteNumb(Integer id) {
        String sql = "update t_place_site set numb=numb+1 where id = ?";
    ...
    @Transactional
    public void save(PlaceSite placeSite) {
    ...
    public Integer getCount(Integer placeId) {
        String sql = "select sum(numb) from t_place_site where place_id = ?";
    ...
    public List<SelectSite> getSelectSite(Integer placeId, Integer cateId) {
            String sql = "select id, name,price,score from t_place_site where place_id = ? and cate_id = ? ORDER BY name asc";
        return generalDao.queryList(SelectSite.class, sql, placeId, cateId);
    }

    @Transactional
    public void updateSite(PlaceSite site) {
    ...
    @Autowired
    private PlaceSiteDao placeSiteDao;
}
