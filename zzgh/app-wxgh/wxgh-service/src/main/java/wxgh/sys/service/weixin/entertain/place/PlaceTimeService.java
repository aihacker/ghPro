...
public class PlaceTimeService {
    ...
    public void updatePlaceTimeClose(String ids, Boolean isSingle, String dateId){
        ...
        if(com.libs.common.type.TypeUtils.empty(ids))
            return ;
        String sql = isSingle ? "update t_place_time set d_status = 4 where id in ( " +ids +")" : "update t_place_time set s_status = 4 where id in ( " +ids +")";
        generalDao.execute(sql);

        // 给予积分预约的用户退回  （条件：积分支付，预约成功）
        String ssql = "select * from t_place_yuyue where date_id = ? and time_id in ("+ids+") and pay_type = 1 and status = 1";
        List<PlaceYuyue> list = generalDao.queryList(PlaceYuyue.class, ssql, dateId);
        // 更新预约状态
        String updateSql = "update t_place_yuyue set status = 4 where date_id = ? and time_id in ("+ids+") and pay_type = 1 and status = 1";
        generalDao.execute(updateSql, dateId);

        // 返回积分
        for (PlaceYuyue placeYuyue : list){
     ...
     * @return
     */
    public List<PlaceTime> getWeekTimeList(Integer week, Integer placeId, Integer cateId, String timeId){
        SQL.SqlBuilder sql = new SQL.SqlBuilder()
                .table("place_time t")
                .field("t.*")
                .join("t_place_site s","s.id = t.site_id")
                .where("s.place_id = ?")
...
//        (start_time < '12:00' OR (start_time >= '12:00' and start_time < '18:00') OR (start_time >= '18:00'))
        ...
        return generalDao.queryList(PlaceTime.class, sql.select().build().sql() + " AND ("+ TypeUtils.listToString(sqls, " OR ")+")", placeId, cateId, week);
    }
    ...
    public void placeExpire(Integer id, Boolean isSingle) {
        ...
        String sql = null;
        if (isSingle)
            sql = "update t_place_time set d_status=? where id = ? and d_status != ?";
        else
            sql = "update t_place_time set s_status=? where id = ? and s_status != ?";
        generalDao.execute(sql, 2, id, 3);

        // 更新预约信息
        String date = DateUtils.formatDate(new Date(), "yyyyMMdd");
        String sql2 = "update t_place_yuyue set status = 3 where status = 1 and date_id = ? and time_id = ?";
    ...
    public void updateStatus(Integer id, Integer status) {
        String sql = "update t_place_time set status=? where id = ? and status != ?";
    ...
    public void updateStatus(List<Integer> ids, Integer status) {
        ...
        String sql = "update t_place_time set status=? where status != ? and status != 3 and id in (" + TypeUtils.listToStr(ids) + ")";
    ...
    public void updateStatus(List<Integer> ids, Integer status, boolean isSingle) {
        ...
        String sql = "";
        if (isSingle)
            sql = "update t_place_time set d_status=? where d_status != 3 and id in (" + TypeUtils.listToStr(ids) + ")";
        else
            sql = "update t_place_time set s_status=? where s_status != 3 and id in (" + TypeUtils.listToStr(ids) + ")";
        generalDao.execute(sql, status);

        // 更新预约信息
        String date = DateUtils.formatDate(new Date(), "yyyyMMdd");
        String sql2 = "update t_place_yuyue set status = 3 where status = 1 and date_id = ? and time_id in (" + TypeUtils.listToStr(ids) + ")";
    ...
    public void updateStatusNot(List<Integer> ids, Integer status) {
        ...
        String sql = "update t_place_time set status=? where status != ? and id not in (" + TypeUtils.listToStr(ids) + ")";
        generalDao.execute(sql, status, status);
    }

    @Transactional
    public void updateWeekStatus(Integer week, Integer status) {
        String sql = "update t_place_time set status=? where week=? and status != 3";
    ...
    public void updateWeekStatus(Integer status, boolean isSingle) {
        ...
        // 查询数据库
        String sql_val = "select val from t_sys_val where name = ?";
        String weekVal = generalDao.query(String.class, sql_val, PlaceSchedule.THIS_WEEK);
        // 如果记录不存在则插入
        if (weekVal == null) {
            String insertSql = "insert into t_sys_val(name, val) values(?, ?)";
            generalDao.execute(insertSql, PlaceSchedule.THIS_WEEK, w);
            weekVal = generalDao.query(String.class, sql_val, PlaceSchedule.THIS_WEEK);
        }
        ...
        String updateVal = "update t_sys_val set val = ? where name = ?";
        generalDao.execute(updateVal, w, PlaceSchedule.THIS_WEEK);

        String sql = null;
        // 如果本周是单周，则更新双周 s_status 如果本周是双周，则更新单周 d_status
        if (!isSingle)
            sql = "update t_place_time set d_status=? where d_status != 3";
        else
            sql = "update t_place_time set s_status=? where s_status != 3";
        generalDao.execute(sql, status);

    }

    public PlaceTime getPlaceTime(Integer id) {
        return placeTimeDao.get(id);
    }

    public PlaceTime getPriceTime(Integer id) {
        String sql = "select t.*, s.price, s.score from t_place_time t\n" +
                "join t_place_site s on t.site_id = s.id where t.id=?";
        return generalDao.query(PlaceTime.class, sql, id);
    }

    // 新增, 区分单双周
    public PlaceTime getPriceTime(Integer id, Integer isSingle) {
        SQL.SqlBuilder sql = new SQL.SqlBuilder()
        ...
        return generalDao.query(PlaceTime.class, sql.build().sql(), id);
    }
    ...
    public List<Integer> getTimeOutTime() {
        ...
        String sql = "select id from t_place_time\n" +
    ...
    public int updateStatusAll(Integer status) {
        String sql = "update t_place_time set status=?";
        return generalDao.execute(sql, status);
    }

    @Transactional
    public void saveTime(PlaceTime placeTime) {
    ...
    public void del(Integer id) {
        ...
        //删除固定场
        String sql = "delete from t_place_guding where time_id = ?";
        generalDao.execute(sql, id);
    }

    public List<SelectTime> getTime(Integer placeId, Integer cateId, Integer week) {
        String sql = "select DISTINCT start_time,end_time from t_place_time t\n" +
...
//    }
    ...
    public List<SelectTime> getTime(Integer siteId, Integer week) {
        String sql = "select start_time, end_time, status, id as timeId, time_type as type from t_place_time where site_id=? and week=? ORDER BY start_time asc";
        return generalDao.queryList(SelectTime.class, sql, siteId, week);
    }

    public List<PlaceTime> getPlaceTimes(Integer placeId, Integer cateId) {
        SQL.SqlBuilder sql = new SQL.SqlBuilder()
                .table("t_place_time t")
                .field("t.*")
                .join("t_place_site s", "t.site_id = s.id")
...
//                "join t_place_site s on t.site_id = s.id\n" +
//                "where s.place_id = ? and t.cate_id = ?";
        return generalDao.queryList(PlaceTime.class, sql.select().build().sql(), placeId, cateId);
    }

    public Map<String, Integer> getTimeIdMap(Integer placeId, Integer cateId) {
        String sql = "select CONCAT(t.`week`,'-', t.start_time, '-', t.end_time, '-',  s.id) as mk, t.id as val from t_place_time t\n" +
                "join t_place_site s on t.site_id = s.id\n" +
                "where s.place_id=? and s.cate_id=?";
    ...
    public void changeStatusById(Integer id, Integer state) {
        PlaceTime placeTime = placeTimeDao.get(id);
        placeTime.setStatus(state);
        SQL sql = new SQL.SqlBuilder()
                .update("t_place_time t")
     ...
     * @return
     */
    public List<PlaceYuyue> getYuYueList(PlaceParam query){
        ...
        List<PlaceYuyue> list = generalDao.queryPage(sql,query,PlaceYuyue.class);
