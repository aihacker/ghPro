package wxgh.sys.service.weixin.union.group.act.result;
...
public class ActResultService {
    ...
    public List<Comment> listComm(CommParam query) {
        ...
        String sql = "select *from t_act_result where id=" + query.getArticleId();
        ActResult actResult = pubDao.query(ActResult.class,sql);
    ...
    public Integer getNumb(Integer id, String colum) {
        String sql = "select " + colum + " from t_act_result where id=?";
    ...
    public void updateSeeNum(Integer id) {
        String sql = "update t_act_result set see_numb = see_numb+1 where id=?";
    ...
    @Transactional
    public Integer addResult(ActResult result) {
        ...
        String sql = "insert into t_act_result(title, info, imgs, act_id, status, add_time, userid, group_id) " +
    ...
    public ResultInfo getResult(Integer id) {
        SQL sql = new SQL.SqlBuilder()
                .table("act_result r")
                ...
                .where("r.id = ?")
                .build();
        ResultInfo info = pubDao.query(ResultInfo.class, sql.sql(), Zan.TYPE_ACT_RESULT, UserSession.getUserid(), id);
    ...
    public List<ResultList> listResult(SignParam param) {
        ...
        if (param.getActId() != null) {
            sql.join("act a", "a.act_id = r.act_id")
                    .where("a.id = :actId");
        }
