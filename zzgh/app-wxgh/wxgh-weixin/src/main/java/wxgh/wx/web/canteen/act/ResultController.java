...
import wxgh.sys.service.weixin.canteen.CanteenActResultService;
...
public class ResultController {
    ...
    public void add(Model model, Integer id) throws WeixinException {
        List<NameValue> act = actResultService.listCanteenAct(id);
        model.put("acts", act);
    ...
    public void show(Integer id, Model model) {
        ...
        CanteenResultInfo result = actResultService.getCanteenResult(id);
        if (result != null) {
    ...
    public ActionResult api_list(SignParam param){
        ...
        List<ResultList> results = actResultService.canteenListResult(param);

        return ActionResult.okRefresh(results, param);
    }

    @RequestMapping
    public ActionResult api_add(CanteenActResult result, Integer pushType) {
        Integer resultId = actResultService.addResult(result);
        weixinPush.act_canteen_result(pushType, resultId);
    ...
    @Autowired
    private CanteenActResultService actResultService;
