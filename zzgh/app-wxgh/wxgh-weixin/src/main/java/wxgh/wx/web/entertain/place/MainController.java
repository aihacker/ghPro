package wxgh.wx.web.entertain.place;
...
import wxgh.app.sys.utils.place.VerifyUtils;
...
import wxgh.data.entertain.place.SelectYuyue;
import wxgh.data.entertain.place.TimeData;
import wxgh.data.entertain.place.yuyue.ItemCell;
import wxgh.data.entertain.place.yuyue.Site;
import wxgh.data.entertain.place.yuyue.TimeCell;
import wxgh.data.entertain.place.yuyue.WeekCell;
import wxgh.data.entertain.place.yuyue.YuyueList;
import wxgh.data.entertain.place.yuyue.YuyueSheet;
...
import wxgh.sys.service.weixin.entertain.place.PlaceGudingService;
import wxgh.sys.service.weixin.entertain.place.PlaceService;
import wxgh.sys.service.weixin.entertain.place.PlaceSiteService;
...
import wxgh.sys.service.weixin.entertain.place.PlaceimgService;
...
public class MainController {
    ...
    public void index(Model model) {
        List<PlaceCate> cates = placeCateService.getCates(1);
    ...
    public void show(Model model, Integer id) {
        ...
        if (!StrUtils.empty(place.getTypeInt())) {
            List<PlaceCate> cates = placeCateService.getPlaceCates(place.getTypeInt());
...
//            }
    ...
    public ActionResult place_list(PlaceParam query) {
        ...
        //默认内部场地
        if (query.getPlaceType() == null) query.setPlaceType(Place.PLACE_TYPE_DEPT);
        ...
        query.setIsAd(1);
        Place place = placeService.getAdPlace(query.getPlaceType(), query.getTypeInt());
...
//                    List<SelectTime> tims = placeTimeService.getTime(sites.get(i).getId(), query.getWeek());
    ...
    public ActionResult get_time(TimeParam query) {
        List<PlaceTime> placeTimes = placeTimeService.getTimes(query);
    ...
    public ActionResult yuyue_json(String json) {
        ...
        List<PlaceYuyue> yuyues = JsonUtils.parseList(json, PlaceYuyue.class);

        List<String> errorMsgs = new ArrayList<>();

        for (PlaceYuyue yuyue : yuyues) {

            PlaceTime time = placeTimeService.getPriceTime(yuyue.getTimeId(), yuyue.getIsSingle());
...
//                                placeTimeService.updateStatus(time.getId(), 1);
    ...
    ActionResult pre_list2(Integer id, Integer cateId) {
        ...
        //获取当前场馆下所有的场地
        List<PlaceSite> placeSites = placeSiteService.getSites(id, cateId);
        List<Site> sites = new ArrayList<>();
        for (PlaceSite site : placeSites) {
            sites.add(new Site(site.getId(), site.getName()));
        }
        yuyueSheet.setSites(sites);

        //获取当前场馆下该分类的全部可预约时间
        List<PlaceTime> placeTimes = placeTimeService.getPlaceTimes(id, cateId);
        Map<Integer, List<PlaceTime>> timeMap = new HashMap<>();
        for (PlaceTime p : placeTimes) {
            List<PlaceTime> tmp = timeMap.getOrDefault(p.getWeek(), null);
            if (tmp == null) {
                tmp = new ArrayList<>();
                tmp.add(p);
    ...
    public ActionResult pre_list(Integer id, Integer cateId) { //分类ID
        ...
        if (!TypeUtils.empty(sites)) {
            ...
            for (int i = 1; i <= 7; i++) {
                ...
                List<PlaceTime> timeList = getTimes(timeMap.get(i));
                if (!TypeUtils.empty(timeList)) {
                    Collections.sort(timeList, new Comparator<PlaceTime>() {
                        @Override
                        public int compare(PlaceTime o1, PlaceTime o2) {
                            return o1.getStartTime().compareTo(o2.getStartTime());
                        }
                    });

                    for (PlaceTime t : timeList) {
    ...
    private List<PlaceTime> getTimes(List<PlaceTime> times) {

        Map<String, PlaceTime> map = new HashMap<>();
        if (times != null)
            for (PlaceTime t : times) {
                map.put(t.getStartTime() + "-" + t.getEndTime(), t);
            }

        List<PlaceTime> tmp = new ArrayList<>();
        for (Map.Entry<String, PlaceTime> entry : map.entrySet()) {
