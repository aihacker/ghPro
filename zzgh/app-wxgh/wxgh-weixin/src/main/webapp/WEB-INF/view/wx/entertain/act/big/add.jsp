            ...
            <div class="mui-scroll">
                ...
                <ul class="mui-table-view ui-margin-top-10">
                    <li class="mui-table-view-cell">
                        ...
                        <select name="actType">
                            <option value="3">工会小组</option><%--本部--%>
                            <%--<option value="4">区分公司</option>--%>
                            <option value="5">公司</option><%--分公司--%>
                        </select>
                    </li>
                    ...
                    <li class="mui-table-view-cell no">
                        <a>
                            <div class="ui-content">
                                ...
                                <div class="ui-text-div">
                                    <textarea name="addrRemark" maxlength="50" rows="1" id="remark"></textarea>
                                </div>
                            </div>
                        </a>
                    </li>
...
//            uploadImg(function (img) {
        ...
        function verify(info) {
            ...
            if (!info['actNumb']) {
                ...
                var reg = /^[1-9]+[0-9]*]*$/;
                if (!reg.test(info['actNumb'])) {
                    return "输入不合法, 参加活动的人数必须为正正数"
                }
            }
        }
        ...
        function ruleInit() {
            ...
            $('#addRuleBtn').on('tap', function () {
                ...
                if (!score['score'] || isNaN(score.score)) {
                    ui.alert('参加积分不能为空或数值不合法, 必须为正数')
                    return
                }
                if (!score['leaveScore'] || isNaN(score.leaveScore)) {
                    ui.alert('请假积分不能为空或数值不合法, 必须为正正数')
                    return
                }
                if (!score['outScore'] || isNaN(score.outScore)) {
                    ui.alert('缺席积分不能为空或数值不合法, 必须为正正数')
                    return
                }
