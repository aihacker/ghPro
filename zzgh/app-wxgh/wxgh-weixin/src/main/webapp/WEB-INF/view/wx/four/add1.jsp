...
<div class="mui-content">
    <ul id="mainUl" class="mui-table-view">
        ...
        <li data-type="id" class="mui-table-view-cell">
            <a class="mui-navigate-right">
                <span>更换的台账：</span>
                <small>请选择</small>
            </a>
        </li>
        <li data-type="fpcids" style="display: none;"></li>
...
<script>
    var _self = homePath + '/wx/four/list_out.json'
    ...
    $(function () {
        ...
        var outFm = new Confirm('选择需要更换台账', loadingHtml, function ($dialog) {
            ...
            if ($li.length > 0) {
                ...
                $('li[data-type=fpcids]').data('data', {fpId: d.fpId, fpcId: d.fpcId})

                $('li[data-type=brand]').data('data', {name:d.brand})
                $('li[data-type=modal]').data('data', {name:d.modelName})
                $("#totalPrice").text(d.numb * d.price + "元")
            ...
            mui.getJSON(_self, {id: mid}, function (res) {
                if (res.ok) {
                    var ds = res.data
                    if (ds && ds.length > 0) {
        ...
        $('#addBtn').on('tap', function () {
            ...
            if(deviceStatus == 2){
                var ids = $('li[data-type=fpcids]').data('data');
                if(ids){
                    info['fpId'] = ids.fpId
                    info['fpcId'] = ids.fpcId
                }
            }
            ...
            info['remark'] = reamark
            var url = homePath + '/wx/four/add_propagate.json'
