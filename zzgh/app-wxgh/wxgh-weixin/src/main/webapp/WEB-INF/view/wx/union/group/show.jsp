...
<div class="mui-content">
    ...
    <div class="group-head">
        ...
        <c:if test="${hasPermission}">
            <div class="ui-change-avatar">点击更换协会头像</div>
        </c:if>
        ...
        <div>
            <ul class="mui-table-view mui-grid-view mui-grid-9">
                <li class="mui-table-view-cell mui-media mui-col-xs-3 mui-col-sm-3">
                    <a href="${home}/wx/union/group/act/index.html?id=${param.id}">
                ...
                <c:if test="${hasPermission}">
                    <li class="mui-table-view-cell mui-media mui-col-xs-3 mui-col-sm-3">
                        <a href="${home}/wx/union/group/apply/index.html?id=${param.id}">
							<span class="fa-stack fa-lg">
							  <i class="fa fa-circle fa-stack-2x"></i>
    ...
    <div class="ui-team-info ui-margin-top-10">
        <h5>协会介绍</h5>
        <small class="ui-text-info">本协会创建于<c:choose><c:when
...
<div id="quitSheet" class="mui-popover mui-popover-bottom mui-popover-action ">
    ...
    <ul class="mui-table-view">
        <li class="mui-table-view-cell ui-text-danger">
            <a href="javascript:;" id="quitBtn">${hasPermission?'解散协会':'退出协会'}</a>
        </li>
    </ul>
    <!-- 取消菜单 -->
    <ul class="mui-table-view">
...
<script>
    ...
    $(function () {
        ...
        $('#quitBtn').on('tap', function () {
            wxgh.request.post('/wx/union/group/show/out.json', {id: groupId}, function (msg) {
                ui.showToast(msg, function () {
                    mui.openWindow(homePath + '/wx/union/group/index.html')
                })
            })
        })
        ...
        $('#applyBtn').on('tap', function () {
            ...
            var cfm
            if (stats && stats == 2) {
                cfm = confirm('是否重新申请加入协会')
            } else {
                cfm = confirm('是否申请加入协会')
            }
            if (cfm) {
                wxgh.request.post('/wx/union/group/show/apply.json', {id: groupId}, function () {
        ...
        $('#groupUserLi').on('tap', function (e) {
            mui.openWindow(homePath + '/wx/union/group/user/index.html?id=' + groupId)
        })
        ...
        function editGroup(info, self) {
            info['id'] = groupId
            wxgh.request.post('/wx/union/group/show/edit.json', info, function () {
                ui.showToast('修改成功')
            })
        }
    })
