...
//                alert(addComp.province + ", " + addComp.city + ", " + addComp.district + ", " + addComp.street + ", " + addComp.streetNumber);
    ...
    function placeImgeAdmin() {
        ...
        function placeImgcallback(rs) {
            ...
            if (data.length > 0) {
                for (var i = 0; i < data.length; i++) {
                    ps = ps + '<div class="col-md-3 col-sm-3 col-xs-2"><a class="thumbnail"href="#"><img style="height: 100px;" src="' + wxgh.get_image(data[i].imgPath) + '"/></a><span data-id="' + data[i].id + '" class="fa fa-remove text-danger ui-delete"></span></div>';
                    if (i == data.length - 1) {
                        ps = ps + '<div class="col-md-3 col-sm-3 col-xs-2"><a class="thumbnail"href="#"><img class="add" src="' + homePath + '/image/xlkai/icon_add1.png"/></a></div>';
                    }
                }
            } else {
                ps = ps + '<div class="col-md-3 col-sm-3 col-xs-2"><a class="thumbnail"href="#"><img class="add" src=homePath+"/image/xlkai/icon_add1.png"/></a></div>';
    ...
    $gudingCateSelect.on('change', function () {
        var val = $(this).val()
        if (val != 0) {
            $.getJSON(self_url, {action: 'getPlaceSite', placeId: myPlaceId, cateId: val}, function (res) {
                if (res.ok) {
                    var sites = res.data
    ...
    function listGuding() {
        ...
        $.getJSON(self_url, info, function (res) {
            if (res.ok) {
                var gs = res.data
                $gudingBody.empty()
                if (gs && gs.length > 0) {
    ...
    $('#gudingCateList').on('click', 'a.btn', function () {
        ...
        if (!$self.hasClass('active')) {
            ...
            $.getJSON(self_url, {action: 'getPlaceSite', placeId: placeId, cateId: id}, function (res) {
                if (res.ok) {
    ...
    $('#addGudingModalBtn').on('click', function () {
        ...
        if (usertype == 1) { //系统用户
            ...
            info = wxgh.serialize(wxgh.getElement('outUserDiv'))
        ...
        $.post(self_url, info, function (res) {
            if (res.ok) {
                alert('添加成功')
                window.location.reload()
            } else {
    ...
    function initgudingEvent($tr) {
        ...
        $tr.on('click', '.ui-del', function () {
            ...
            var cm = confirm('是否删除？')
            if (cm) {
                $.getJSON(self_url, {action: 'del_guding', id: id}, function (res) {
                    if (res.ok) {
                        $parent.remove();
                    } else {
                        alert('删除失败！')
        ...
        $tr.on('click', '.ui-status', function () {
            ...
            if (cf) {
                $.getJSON(self_url, {action: 'edit_guding', id: id, status: status}, function (res) {
                    if (res.ok) {
                        if (res.data == 1) {
                            $self.text('取消场地');
                        } else {
